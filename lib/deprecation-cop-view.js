...
export default class DeprecationCopView {
      ...
      return packageNames.sort().map((packageName) => (
        <li className='deprecation list-nested-item collapsed'>
          ...
          <ul className='list'>
            ...
            {this.renderPackageDeprecations(packageName, deprecationsByPackageName[packageName])}
          </ul>
        </li>
      ))
    }
  ...
  renderPackageDeprecations (packageName, deprecations) {
    return deprecations.map(({deprecation, stack}) => (
        ...
        {stack.map(({functionName, location}) => (
          <div className='stack-line'>
            ...
            <a
              ...
              onclick={(event) => {
                event.preventDefault()
                this.openStackLineLocation(location)
              }}>{location}</a>
          </div>
        ))}
        </div>
  ...
  openStackLineLocation (location) {
